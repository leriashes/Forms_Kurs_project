...
//		srand(time(0));
			...
			else 
			{
				if ((lines->Length - 10) % 129 == 0)            //форматирование верно
				{
					...
					if (file_stream->kol_vo_film > 0)
					{
						...
						{
							this->PayToolStripMenuItem->Text = "&Аннулирование забронированных билетов";
						}
						else
						{
							this->PayToolStripMenuItem->Text = L"О&плата забронированных билетов";
	...
	private: System::Void DelMovieToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (p->DialogResult == System::Windows::Forms::DialogResult::OK)
		{
			String^ message = L"Вы уверены, что хотите удалить фильм? Это действие невозможно отменить.";
			String^ caption = L"";
	...
	private: System::Void PayToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (num != -1)
		{
			...
			if (film >= 0 && day >= 0 && time >= 0)
			{
				...
				else if (MessageBox::Show(L"Аннулировать выбранные билеты? Это действие невозожно отменить.", "", MessageBoxButtons::YesNo) == System::Windows::Forms::DialogResult::Yes)
				{
					ready = true;
				}
